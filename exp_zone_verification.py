#!/usr/bin/env python3
"""
E75: Zone Verification at Key Doses — All 5 zones at D0, D5, D7

Kimi CONTRADICT #24 claims full-chain plateau at D5 is probe saturation artifact,
not identity rerouting. Predicts transition/relay zones carry noise, not identity.

Discriminating test: measure ALL 5 zones at D0, D5, D7.
  If transition/relay show ELEVATED identity at D5 (vs D0): rerouting confirmed
  If transition/relay show NO identity change at D5: probe saturation confirmed

Also tests D7 regime: does the "distribution mode" (low gap, high erank, high id)
extend to transition/relay zones?

Design: 3 doses × 5 zones × Mistral = 15 zone-Jacobian measurements
Uses same methodology as E72 (zone sub-product Jacobian) + E74 (CCS dosing).

Usage:
  OMP_NUM_THREADS=16 PYTHONUNBUFFERED=1 python3 exp_zone_verification.py --model mistralai/Mistral-7B-Instruct-v0.3
"""
...
RESULTS_DIR = Path(__file__).parent / "results" / "e75_zone_verification"
...
def run_experiment(model_name):
    ...
    # Kimi #24 discriminating analysis
    print(f"\n=== KIMI #24 DISCRIMINATING TEST ===")
    print(f"If transition/relay identity RISES at D5 vs D0: rerouting confirmed")
    print(f"If transition/relay identity FLAT at D5 vs D0: probe saturation confirmed\n")
    for zone_name in ["transition", "relay"]:
        d0 = all_results["D0"][zone_name]
        d5 = all_results["D5"][zone_name]
        d7 = all_results["D7"][zone_name]
        change_d5 = (d5["identity_alignment"] - d0["identity_alignment"]) / max(d0["identity_alignment"], 1e-6)
        change_d7 = (d7["identity_alignment"] - d0["identity_alignment"]) / max(d0["identity_alignment"], 1e-6)
        print(f"  {zone_name}: D0={d0['identity_alignment']:.4f} → D5={d5['identity_alignment']:.4f} ({change_d5:+.0%}) → D7={d7['identity_alignment']:.4f} ({change_d7:+.0%})")

    # Inter-zone coherence analysis (F256 test)
    print(f"\n=== INTER-ZONE COHERENCE (principal angle between zone v₁ vectors) ===")
    zone_pairs = [
        ("decouple", "transition"),
    ...
    for dose in DOSES:
        ...
        for z1, z2 in zone_pairs:
            v1 = np.array(all_results[f"D{dose}"][z1]["top_5_Vt"][0])
            v2 = np.array(all_results[f"D{dose}"][z2]["top_5_Vt"][0])
            cos_angle = abs(np.dot(v1, v2))
            angle_deg = np.degrees(np.arccos(np.clip(cos_angle, 0, 1)))
            print(f"    {z1:>12}↔{z2:<12} angle={angle_deg:.1f}° (cos={cos_angle:.4f})")
    ...
    # Save
    summary = {
        "experiment": "E75_zone_verification",
        "model": model_name,
        "doses": DOSES,
