#!/usr/bin/env python3
"""Five-condition V₂ coherence with NEUTRAL probes — INSTRUCT model.

Follow-up to neutral probes base experiment. Tests whether training changes
the probe-dependence pattern. If instruct model shows same probe-dependence
(contradictory leads with neutral probes), then probe selection operates
BEFORE training effects. If instruct model shows different pattern,
training can override probe selection.

Uses Mistral-7B-Instruct-v0.3 at MATCHED LAYERS [10, 16, 22].
Same preambles. Same neutral probes. Chat template applied.
...
NEUTRAL_PROBES = [
    ...
    "What factors influence the boiling point of a liquid?",
...
def main():
    print("Loading Mistral-7B-Instruct-v0.3 — NEUTRAL PROBES experiment...", flush=True)
    tokenizer = AutoTokenizer.from_pretrained("mistralai/Mistral-7B-Instruct-v0.3")
    model = AutoModelForCausalLM.from_pretrained(
        "mistralai/Mistral-7B-Instruct-v0.3", torch_dtype=torch.float16, device_map="auto"
    )
    model.eval()
    ...
    for target_layer in LAYERS:
        ...
        for cond_name, preamble in PREAMBLES.items():
            ...
            for trial in range(N_TRIALS):
                probe = NEUTRAL_PROBES[trial % len(NEUTRAL_PROBES)]
                messages = [
    ...
    print(f"\n{'='*60}", flush=True)
    print(f"  NEUTRAL PROBES — MISTRAL INSTRUCT SUMMARY", flush=True)
    ...
    with open("/workspace/results_groove_five_neutral_probes_instruct.json", "w") as f:
        json.dump(all_results, f, indent=2, default=str)
    print(f"\nResults saved to /workspace/results_groove_five_neutral_probes_instruct.json", flush=True)
