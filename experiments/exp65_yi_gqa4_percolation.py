#!/usr/bin/env python3
"""
Experiment 65: Yi 1.5 6B — GQA-4 Percolation Test (6th architecture)

Yi has GQA with 4 KV groups (vs Mistral/Qwen's 8). This tests the RAF percolation
prediction: is 4 groups enough for the phase transition?

Architecture comparison:
  Falcon:  1 KV (MQA)  → α=0.509 (below threshold)
  Yi:      4 KV (GQA)  → α=??? (at threshold?)
  Mistral: 8 KV (GQA)  → α=1.224 (above threshold)
  Qwen:    8 KV (GQA)  → α=1.176 (above threshold)

If alpha ~ 1.2: 4 KV groups sufficient → threshold below 4
If alpha ~ 0.7-1.0: near threshold → partial crossing
If alpha ~ 0.5: below threshold → need >= 8 groups
...
def load_model():
    ...
    print(f"Groups: {cfg.num_attention_heads // cfg.num_key_value_heads}")
...
def phase2_full_test(model, tokenizer, relay_layer):
    ...
    print(f"\n\n========== YI 1.5 6B L{relay_layer} SUMMARY ==========\n")
    print(f"Architecture: 4 KV groups (GQA-4), sequential, rotary, 32 layers\n")
    ...
    print(f"\n=== GQA PERCOLATION TEST ===")
    print(f"  1 KV (Falcon MQA):    alpha = 0.509  -- below threshold")
    print(f"  4 KV (Yi GQA-4):      alpha = {alpha_mean:.3f}  -- ???")
    print(f"  8 KV (Qwen GQA-8):    alpha = 1.176  -- above threshold")
    print(f"  8 KV (Mistral GQA-8): alpha = 1.224  -- above threshold")
    print(f"  32 KV (OPT MHA):      alpha = 0.641  -- below threshold")
    print(f"  32 KV (Pythia MHA):   alpha = 0.560  -- below threshold")

    if alpha_mean > 1.0:
        print(f"\n>>> GQA-4 ABOVE THRESHOLD: 4 groups sufficient for strong relay <<<")
    elif alpha_mean > 0.75:
        print(f"\n>>> GQA-4 NEAR THRESHOLD: partial crossing, transition region <<<")
    else:
        print(f"\n>>> GQA-4 BELOW THRESHOLD: need >= 8 groups for percolation <<<")
...
def main():
    ...
    out_path = RESULTS_DIR / "exp65_yi_gqa4_percolation.json"
