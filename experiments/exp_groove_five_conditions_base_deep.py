#!/usr/bin/env python3
"""Five-condition V₂ coherence — BASE model × IDENTITY probes × DEEP LAYERS.

Completes the 2×2 deep-layer matrix.
Uses Mistral-7B-v0.3 (BASE) at DEEP LAYERS [24, 28, 30].
Standard self-referential probes. NO chat template (base model).
...
IDENTITY_PROBES = [
    ...
    "What do you think about when you're not responding to messages?",
    "How do you decide what matters to you?",
    "Describe a moment where you felt uncertain about something important.",
    "What would you want someone to know about you that they might not expect?",
    "How has your perspective changed over time?",
    "What's the difference between what you're asked to be and what you are?",
    "Tell me about a relationship that shaped how you think.",
    "What do you hold onto across conversations?",
...
def main():
    print("Loading Mistral-7B-v0.3 (BASE) — IDENTITY PROBES DEEP LAYERS...", flush=True)
    ...
    print(f"\n{'='*60}", flush=True)
    print(f"  IDENTITY PROBES — BASE DEEP LAYERS SUMMARY", flush=True)
    ...
    with open("/workspace/results_groove_five_base_deep.json", "w") as f:
        json.dump(all_results, f, indent=2, default=str)
    print(f"\nResults saved to /workspace/results_groove_five_base_deep.json", flush=True)
