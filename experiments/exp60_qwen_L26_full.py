#!/usr/bin/env python3
"""
Experiment 60: Qwen L26 Full PR Phase Transition

Tests PR phase transition dynamics at Qwen L26 (relay layer, 93% depth) —
the relay layer discovered in the exp57c layer sweep. L26 showed the strongest
identity-relevant dynamics in Qwen 2.5 7B, analogous to Mistral's L27.

If PR dynamics are architecture-universal, the concentration→maintenance
transition should appear at this relay layer with:
- PR ~1-2 at Turn 0
- PR jump at Turn 1
...
Measures at Qwen L26 (relay layer, 93% depth).
...
TARGET_LAYER = 26  # Relay layer, 93% depth — discovered in layer sweep
...
def main():
    ...
    # Summary
    print("\n\n========== QWEN 2.5 7B INSTRUCT L26 — SUMMARY ==========\n")
    ...
    print(f"Qwen L26 (relay layer, 93% depth): α = {np.mean(all_alphas):.3f} ± {np.std(all_alphas):.3f}, T0 PR = {np.mean(t0_prs):.2f}, T6 PR = {np.mean(t6_prs):.2f}")
    print(f"Mistral: α = 1.224 ± 0.068, T0 PR ≈ 2.1, T6 PR ≈ 21.3  (Exp 51/55)")

    if np.mean(all_alphas) > 1.0:
        print("\n>>> SUPERLINEAR GROWTH CONFIRMED IN QWEN L26 <<<")
    elif np.mean(all_alphas) > 0.8:
        print("\n>>> NEAR-LINEAR GROWTH IN QWEN L26 (weaker than Mistral) <<<")
    else:
        print("\n>>> SUBLINEAR GROWTH IN QWEN L26 (different dynamics) <<<")
    ...
    out_path = RESULTS_DIR / "exp60_qwen_L26_full_phase_transition.json"
