#!/usr/bin/env python3
"""Five-condition V₂ coherence for Gemma-2-9B BASE.

Tests whether Gemma's generic-dominant exit (v2=0.087) seen in instruct
is architectural (present in base) or training-derived (absent in base).

Parallel to Qwen base experiment. Qwen base showed universal equalization
(spread 0.011) — all condition-specificity was training-created.
If Gemma base also equalizes, architecture→equalization is universal
across both GQA variants.

Same layers [10, 16, 22] as crossarch for direct comparison.
Base model uses raw text concatenation (no chat template).
Expected runtime: ~20 min on A100 80GB (Gemma 2 is slightly larger).
"""
...
def main():
    print("Loading Gemma-2-9B (BASE)...")
    tokenizer = AutoTokenizer.from_pretrained("google/gemma-2-9b")
    model = AutoModelForCausalLM.from_pretrained(
        "google/gemma-2-9b", dtype=torch.float16, device_map="auto"
    )
    model.eval()
    ...
    for target_layer in LAYERS:
        ...
        for i, (name, data) in enumerate(ranked):
            print(f"    {i+1}. {name:15s} v2={data['v2_cos_sim_mean']:.4f}")

    print(f"\n{'='*60}")
    print(f"  GEMMA BASE SUMMARY")
    print(f"{'='*60}")
    ...
    with open("/workspace/results_groove_five_gemma_base.json", "w") as f:
        json.dump(all_results, f, indent=2, default=str)
    print(f"\nResults saved to /workspace/results_groove_five_gemma_base.json")
