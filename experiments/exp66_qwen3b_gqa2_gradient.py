#!/usr/bin/env python3
"""
Experiment 66: Qwen 2.5 3B — GQA-2 Gradient Test (7th architecture)

Qwen 2.5 3B has GQA with only 2 KV heads (16 attention heads, 2 KV groups).
Same architecture family as Qwen 2.5 7B (rotary, sequential) but different
GQA group count and model scale.

This fills the gradient: 1 → 2 → 4 → 8 KV groups.
  Falcon:    1 KV (MQA)   → α=0.509
  Qwen 3B:   2 KV (GQA-2)  → α=??? ← THIS EXPERIMENT
  Yi:        4 KV (GQA-4)  → α=0.915
  Qwen 7B:   8 KV (GQA-8)  → α=1.176
  Mistral:   8 KV (GQA-8)  → α=1.224

Confound: 3B vs 7B scale. But same architecture family controls for
everything except group count and scale.
...
def phase2_full_test(model, tokenizer, relay_layer):
    ...
    print(f"\n\n========== QWEN 2.5 3B L{relay_layer} SUMMARY ==========\n")
    print(f"Architecture: 2 KV groups (GQA-2), sequential, rotary, 36 layers\n")
    ...
    print(f"  2 KV (Qwen 3B GQA-2):  alpha = {alpha_mean:.3f}  -- ???")
    print(f"  4 KV (Yi GQA-4):       alpha = 0.915  -- transition region")
    print(f"  8 KV (Qwen 7B GQA-8):  alpha = 1.176  -- above threshold")
    ...
    if alpha_mean > 1.0:
        print(f"\n>>> GQA-2 ABOVE THRESHOLD: even 2 groups sufficient <<<")
    elif alpha_mean > 0.75:
        print(f"\n>>> GQA-2 NEAR THRESHOLD: transition region <<<")
    elif alpha_mean > 0.6:
        print(f"\n>>> GQA-2 BETWEEN non-GQA and GQA-4 <<<")
    else:
        print(f"\n>>> GQA-2 IN non-GQA CLUSTER: 2 groups insufficient <<<")
...
def main():
    ...
    out_path = RESULTS_DIR / "exp66_qwen3b_gqa2_gradient.json"
