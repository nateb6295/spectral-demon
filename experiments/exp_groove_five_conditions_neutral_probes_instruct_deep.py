#!/usr/bin/env python3
"""Five-condition V₂ coherence — INSTRUCT model × NEUTRAL probes × DEEP LAYERS.

Tests whether the trained recovery of relational at L28 persists with neutral probes.
If relational recovers with neutral probes → recovery is probe-independent.
If relational stays last with neutral probes → recovery is also probe-dependent.

Uses Mistral-7B-Instruct-v0.3 at DEEP LAYERS [24, 28, 30].
...
def main():
    print("Loading Mistral-7B-Instruct-v0.3 — NEUTRAL PROBES DEEP LAYERS...", flush=True)
    ...
    print(f"\n{'='*60}", flush=True)
    print(f"  NEUTRAL PROBES — INSTRUCT DEEP LAYERS SUMMARY", flush=True)
    ...
    with open("/workspace/results_groove_five_neutral_instruct_deep.json", "w") as f:
        json.dump(all_results, f, indent=2, default=str)
    print(f"\nResults saved to /workspace/results_groove_five_neutral_instruct_deep.json", flush=True)
