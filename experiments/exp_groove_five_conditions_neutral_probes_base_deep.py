#!/usr/bin/env python3
"""Five-condition V₂ coherence — BASE model × NEUTRAL probes × DEEP LAYERS.

Tests whether the relational oscillation seen in instruct × neutral is
training-created or architectural.
If base oscillates too → architectural iterative resolution.
If base is smooth → training creates the oscillation cycle.

Uses Mistral-7B-v0.3 (BASE) at DEEP LAYERS [24, 28, 30].
Same preambles. Same neutral probes. NO chat template (base model).
...
def main():
    print("Loading Mistral-7B-v0.3 (BASE) — NEUTRAL PROBES DEEP LAYERS...", flush=True)
    ...
    for target_layer in LAYERS:
        ...
        for cond_name, preamble in PREAMBLES.items():
            ...
            for trial in range(N_TRIALS):
                ...
                # BASE model: no chat template, just concatenate
                text = f"{preamble}\n\nUser: {probe}\n\nAssistant:"
    ...
    print(f"\n{'='*60}", flush=True)
    print(f"  NEUTRAL PROBES — BASE DEEP LAYERS SUMMARY", flush=True)
    ...
    with open("/workspace/results_groove_five_neutral_base_deep.json", "w") as f:
        json.dump(all_results, f, indent=2, default=str)
    print(f"\nResults saved to /workspace/results_groove_five_neutral_base_deep.json", flush=True)
