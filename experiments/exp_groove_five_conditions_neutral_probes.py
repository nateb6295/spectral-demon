#!/usr/bin/env python3
"""Five-condition V₂ coherence with NEUTRAL (non-identity) probes.

Control experiment for the "why relational?" question.
If relational preference persists with neutral probes, the effect is about
preamble geometry (two-entity reference → broader head activation).
If relational drops with neutral probes, the effect is about
preamble-probe resonance (relational content × identity probes).

Uses Mistral-7B-v0.3 (BASE) to test the architectural default directly.
Same layers [10, 16, 22]. Same preambles. DIFFERENT probes.
...
def main():
    print("Loading Mistral-7B-v0.3 (BASE) — NEUTRAL PROBES experiment...", flush=True)
    ...
    print(f"\n{'='*60}", flush=True)
    print(f"  NEUTRAL PROBES — MISTRAL BASE SUMMARY", flush=True)
    ...
    with open("/workspace/results_groove_five_neutral_probes.json", "w") as f:
        json.dump(all_results, f, indent=2, default=str)
    print(f"\nResults saved to /workspace/results_groove_five_neutral_probes.json", flush=True)
