#!/usr/bin/env python3
"""Five-condition V₂ coherence through the relay zone.

Extends §2.4's two-condition (identity/relational) coherence inversion to all
five preamble conditions from §2.1. Tests whether the inversion is specific to
the identity/relational pair or part of a richer transformation pattern.

Mistral-7B-Instruct-v0.3 only. Layers [20, 24, 28] — same as prior experiments.
50 trials per condition per layer = 750 total forward passes.
"""
...
def main():
    print(f"\n{'='*60}")
    print(f"  FIVE-CONDITION V₂ COHERENCE — {MODEL}")
    print(f"{'='*60}")
    ...
    print(f"\n{'='*60}")
    print(f"  FIVE-CONDITION SUMMARY")
    print(f"{'='*60}")
    ...
    with open("/workspace/results_groove_five_conditions.json", "w") as f:
        json.dump(all_results, f, indent=2, default=str)
    print(f"\nResults saved to /workspace/results_groove_five_conditions.json")
