#!/usr/bin/env python3
"""
Falcon 7B witness experiment: second non-GQA control.
Falcon uses MHA + rotary + SEQUENTIAL blocks (unlike Pythia's parallel).
If Falcon also shows no witness effect, GQA is confirmed as the variable,
not parallel vs sequential block architecture.
"""
...
RELAY_LAYER = 30  # ~94% depth, matches Falcon relay from exp 62 (α=0.509 at L30)
...
def main():
    print(f"Falcon 7B Witness Experiment (non-GQA control #2)")
    ...
    print(f"\n=== RESULTS: Falcon 7B (non-GQA, sequential blocks) ===\n")
    ...
    print(f"\n  Between/within: {ratio:.1f}×")

    RESULTS_DIR.mkdir(parents=True, exist_ok=True)
    outfile = RESULTS_DIR / f"exp_witness_falcon_{datetime.now().strftime('%Y%m%d_%H%M')}.json"
    with open(outfile, "w") as f:
        json.dump({
            "model": MODEL_NAME,
            "architecture": "MHA (no GQA), rotary, sequential blocks, instruct-tuned",
