#!/usr/bin/env python3
"""E7: Linear weight interpolation — basin structure test.

Origin: Kimi CONTRADICT (2026-06-16) — LoRA rank conflates adapter capacity
with basin geometry. The correct test: interpolate between base and IT weights.

W(α) = (1-α)·W_base + α·W_IT, α ∈ [0, 0.05, 0.1, 0.2, ..., 0.9, 0.95, 1.0]

Run species kit metrics at each interpolation point.
- Monotonic M3 relay → same basin, manifold evolution
- M3 relay barrier/discontinuity → different basins, phase transition

Also monitor perplexity — if it spikes at intermediate α, that's evidence
for basin separation (the interpolated model passes through a high-loss region).
...
IT_MODEL = "Qwen/Qwen2.5-7B-Instruct"
...
OUTPUT_DIR = Path("/workspace/results/e7")
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)


def load_weights(model_name):
    """Load model weights as state dict."""
...
def load_interpolated_model(sd_interp, model_name):
    """Load a model architecture and inject interpolated weights."""
    from transformers import AutoModelForCausalLM
    model = AutoModelForCausalLM.from_pretrained(
...
def collect_masks(model, tokenizer, num_layers):
    """Collect gate activation masks for all conditions × prompts × layers."""
    all_masks = {}
...
def classify(m2, m3, num_layers):
    ...
    # Updated classifier (E1 fix: no crossovers requirement for goldsmith)
...
def compute_perplexity(model, tokenizer):
    """Quick perplexity check on a fixed string."""
...
def run_alpha(model, tokenizer, num_layers, alpha):
    """Run full species analysis at one interpolation point."""
...
def main():
    ...
    print("=" * 70)
    print("  E7: Weight Interpolation — Basin Structure Test")
    print("=" * 70)

    # Load tokenizer (same for base and IT)
    print("\nLoading tokenizer...")
    tokenizer = AutoTokenizer.from_pretrained(IT_MODEL, trust_remote_code=True)

    # Load both weight sets in fp32 on CPU
    ...
    for alpha in ALPHAS:
        ...
        # Save incremental
        out_path = OUTPUT_DIR / f"e7_alpha_{alpha:.2f}.json"
    ...
    # Summary
    print(f"\n{'='*70}")
    print(f"  E7 SUMMARY — Weight Interpolation")
    print(f"{'='*70}")
    ...
    if ppl_max_interior > 2 * ppl_endpoints:
        print(f"\n  ⚠ PERPLEXITY BARRIER: interior max {ppl_max_interior:.1f} > 2× endpoint avg {ppl_endpoints:.1f}")
        print(f"    → Evidence for BASIN SEPARATION")
    else:
        print(f"\n  No perplexity barrier (interior max {ppl_max_interior:.1f}, endpoint avg {ppl_endpoints:.1f})")
        print(f"    → Consistent with MANIFOLD EVOLUTION")

    # Save combined
    combined = {
        "experiment": "E7_weight_interpolation",
    ...
    combined_path = OUTPUT_DIR / f"e7_combined_{datetime.now():%Y%m%d_%H%M%S}.json"
