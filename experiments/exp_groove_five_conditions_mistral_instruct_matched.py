#!/usr/bin/env python3
"""Five-condition V₂ coherence for Mistral-7B-Instruct-v0.3 at MATCHED LAYERS [10,16,22].

The base experiment used L10/L16/L22. The original instruct experiment used L20/L24/L28.
This creates a matched-layer comparison to close the caveat that spread differences
might reflect layer position rather than training effects.
...
def main():
    print("Loading Mistral-7B-Instruct-v0.3 (INSTRUCT, matched layers)...", flush=True)
    ...
    print(f"\n{'='*60}", flush=True)
    print(f"  MISTRAL INSTRUCT (MATCHED LAYERS) SUMMARY", flush=True)
    ...
    with open("/workspace/results_groove_five_mistral_instruct_matched.json", "w") as f:
        json.dump(all_results, f, indent=2, default=str)
    print(f"\nResults saved to /workspace/results_groove_five_mistral_instruct_matched.json", flush=True)
