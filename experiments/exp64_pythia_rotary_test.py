#!/usr/bin/env python3
"""
Experiment 64: Pythia-6.9B — Rotary Isolation Test (5th architecture)

Pythia has partial rotary (25% of head dims) + full MHA (32 heads) + parallel attn+MLP.

Variable isolation:
  OPT:    no rotary  + MHA + sequential → α=0.641
  Pythia: 25% rotary + MHA + parallel   → α=???
  Falcon: no rotary  + MQA + parallel   → α=0.509
  Mistral: full rotary + GQA + sequential → α=1.224

If alpha > 0.8: even partial rotary helps → rotary hypothesis supported
If alpha ~ 0.6: partial rotary ≈ no rotary → need full rotary, or GQA matters more
...
def load_model():
    ...
    print(f"Rotary pct: {getattr(cfg, 'rotary_pct', 'N/A')}")
    print(f"Parallel residual: {getattr(cfg, 'use_parallel_residual', 'N/A')}")
...
def phase2_full_test(model, tokenizer, relay_layer):
    ...
    print(f"\n\n========== PYTHIA 6.9B L{relay_layer} SUMMARY ==========\n")
    print(f"Architecture: 32 MHA heads, parallel attn+MLP, 25% rotary, 32 layers\n")
    ...
    print(f"Pythia  L{relay_layer}: alpha = {alpha_mean:.3f} +/- {alpha_std:.3f}  (32 MHA, parallel, 25% rotary)")
    print(f"OPT    L12: alpha = 0.641 +/- 0.166  (32 MHA, sequential, no rotary)")
    print(f"Falcon L30: alpha = 0.509 +/- 0.096  (1 MQA, parallel, ALiBi)")
    print(f"Qwen   L26: alpha = 1.176 +/- 0.057  (8 GQA, sequential, full rotary)")
    print(f"Mistral L27: alpha = 1.224 +/- 0.068 (8 GQA, sequential, full rotary)")

    print(f"\n=== ROTARY DOSE-RESPONSE ===")
    print(f"  No rotary (OPT learned):  alpha = 0.641")
    print(f"  No rotary (Falcon ALiBi): alpha = 0.509")
    print(f"  25% rotary (Pythia):      alpha = {alpha_mean:.3f}")
    print(f"  Full rotary + GQA-8:      alpha = 1.176-1.224")
...
def main():
    ...
    output = {
        ...
        "architecture": {
            ...
            "attn_mlp": "parallel",
            "positional_encoding": "rotary_25pct",
            "rotary_pct": 0.25,
        },
        "sweep": {
    ...
    out_path = RESULTS_DIR / "exp64_pythia_rotary_test.json"
