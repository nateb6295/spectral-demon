#!/usr/bin/env python3
"""Qwen 1.5B binding CV — same layer count as 7B but 4.5x narrower.
Tests: is binding about layer depth or parameter capacity?"""
...
# Same layers as 7B for direct comparison
LAYERS = [9, 12, 14, 15, 16, 17, 18, 20, 22, 25, 27]
...
def main():
    ...
    print(f"Model has {len(mdl.model.layers)} layers, hidden={mdl.config.hidden_size}", flush=True)
    ...
    for p in ["What is the capital of France?", "Explain photosynthesis briefly.",
    ...
    print(f"\n{'Layer':<8} {'Depth%':<8} {'genPR':<8} {'idPR':<8} {'CV':<10}", flush=True)
    print("-" * 44, flush=True)
    results = {"model": MODEL_NAME, "n_layers": 28, "hidden_size": 1536}
    layer_results = {}
    for l in LAYERS:
        ...
        gpr = gen_pr or 0
        ipr = id_pr or 0
        print(f"L{l:<7} {depth:<8.1f} {gpr:<8.2f} {ipr:<8.2f} {cv['mean_cv']:<10.4f}", flush=True)

    results["layer_results"] = layer_results
    with open("/workspace/cna_qwen1_5b_binding.json", "w") as f:
        json.dump(results, f, indent=2, default=str)
    print("\nSaved to /workspace/cna_qwen1_5b_binding.json")
