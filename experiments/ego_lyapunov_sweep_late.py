#!/usr/bin/env python3
"""F499b — Ego Lyapunov Exponent (LATE LAYERS): fine-grained KV perturbation sweep.

Repeat F499 on layers 24-31 to test whether KV4's apparent robustness is real
or an artifact of testing outside its functional band. F497 showed KV3/KV4
are critical in late layers but not early. This tests the prediction.

Hypothesis: KV4 epsilon_c should be LOWER in late band than early (4.25x -> ~2.5x?).
KV5 gate may not manifest here since its function is early-band.
"""
...
N_GROUPS = 8
LAYER_BAND = (24, 32)  # late band (execution layers)
MAX_NEW_TOKENS = 256
SEEDS = [42, 137, 2049]
...
def main():
    ...
    # Save
    outpath = Path("/root/results/ego_lyapunov_sweep_late.json")
